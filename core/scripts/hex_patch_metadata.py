...
# elpm = 0xD8 0x95 = 0x95D8  --> does not exist in Atmega328P
# lpm = 0xC8 0x95 = 0x95C8

# Unsafe ops with arguments
# -------------------------
# elpm Rd,Z = op & (0xFE0F) == 0x9006 --> does not exist in Atmega328P
# elpm Rd,Z+ = op & (0xFE0F) == 0x9007 --> does not exist in Atmega328P

# lpm Rd,Z = op & (0xFE0F) == 0x9004
# lpm Rd,Z+ = op & (0xFE0F) == 0x9005
...
unsafe_ops = [int("0x9508", 16), int("0x9518", 16), int("0x9409", 16),
        int("0x9509", 16), int("0x95C8",16)]
lpm_mask = int("0xFE0F", 16)
lpm_ops = [int("0x9004", 16), int("0x9005", 16)]
...
# class
#metadata_offset = int("0x3D00", 16)//2 # 1Kb bootloader
#metadata_offset = int("0x3B00", 16)//2 # 2Kb bootloader
metadata_offset = int("0x3700", 16)//2 # 4Kb bootloader
   ...
   for addr in range(datastart//2):
       if ih[addr] in two_word_ops and not prev_op_long:
           ...
           if ( (ih[addr] & branch_ops_mask) in branch_ops or
                ...
                (ih[addr] & lpm_mask) in lpm_ops or
